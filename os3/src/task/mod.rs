...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.firt_start_time = get_time_us();
    ...
    fn mark_current_exited(&self) {
        ...
        inner.tasks[current].firt_start_time = get_time_us()
    }
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].firt_start_time == 0 {
                inner.tasks[next].firt_start_time = get_time_us();
            }
    ...
    fn update_task_syscall(&self, syscall: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
...
pub fn update_task_syscall(syscall: usize) {
    TASK_MANAGER.update_task_syscall(syscall);
}
